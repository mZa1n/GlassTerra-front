...
| Панель управления целиком, на моке | `src/pages/Admin.tsx`, `src/components/admin/` |
...
Контракт панели управления (`/admin/*`, раздел 5) объявлен в `Backend.admin` и
реализован в обоих транспортах, так что серверу остаётся повторить поведение,
...
model User {
  ...
  phone     String?
  address   String?
  /** Права выдаёт магазин, а не провайдер: см. раздел 2.4. */
...
**Роль — колонка в БД, а не клейм провайдера.** Провайдер отвечает на вопрос
«кто это», магазин — на вопрос «что ему можно». Иначе права зависят от
настроек Keycloak и не переживают смену провайдера.

Фронт уже это ожидает: `User.role` (`"user" | "admin"`) есть в доменном типе,
`toUser` в `dto.ts` при отсутствии поля ставит `"user"` — сервер, забывший
роль, не выдаёт прав. Проверки роли в компонентах решают только, что рисовать;
каждый вызов `api.admin.*` авторизуется заново на сервере.

Отдельное правило, которое стоит перенести с мока: **нельзя снять роль
администратора с самого себя** — иначе панель закрывается за аккаунтом,
которому её уже не вернуть без правки в базе.
...
model Category {
  /**
   * Слаг. Семь стартовых — glasses, mugs, bowls, pitchers, plates, teasets,
   * decor — но набор открытый: категории создаются в панели управления.
   * Слаг неизменяем, на нём построены фильтры каталога.
   */
  id       String    @id
  name     String
  imageUrl String
  /** Заголовок группы в боковом меню, например «Посуда из стекла». */
  group    String?
...
/** Баннеры и заметки страницы «Новости». Редактируются в панели управления. */
model Article {
  id        String      @id @default(uuid())
  kind      ArticleKind
  ...
  /** "page:<slug>" или "filter:<filterKey>" — формат из src/lib/catalog.ts. */
  ctaTarget String      @default("")
  /** Ровно один баннер главный; publish-логика снимает флаг с остальных. */
...
/** Тексты разделов: доставка, контакты, о компании и остальные. */
model PageContent {
  slug      String   @id
  title     String
  intro     String
  /** Массив { heading, body } — порядок значим, поэтому JSONB. */
...
| `GET` | `/articles` | Публично. Только опубликованные, в порядке `sort_order` |

### Панель управления — только роль `ADMIN`

Отдельный префикс, а не флаг у публичных путей: другие правила кеша, отдельный
рейт-лимит и один гард на весь префикс вместо проверки в каждом обработчике.
Любой из этих путей без прав — `403`.

| Метод | Путь | Тело / примечание |
| --- | --- | --- |
| `POST` | `/admin/products` | `ProductDto` без `id`, `rating`, `reviews_count` |
| `PATCH` | `/admin/products/{id}` | Частичное обновление |
| `DELETE` | `/admin/products/{id}` | |
| `POST` | `/admin/categories` | `{ id, name, image_url, group }`, `id` — слаг |
| `PATCH` | `/admin/categories/{id}` | Слаг неизменяем: на нём построены фильтры |
| `DELETE` | `/admin/categories/{id}?move_to=` | Непустую — только с `move_to` |
| `GET` | `/admin/articles` | Включая черновики |
| `POST` `PATCH` `DELETE` | `/admin/articles[/{id}]` | |
| `GET` | `/admin/pages` | `[{ slug, title }]` |
| `PUT` | `/admin/pages/{slug}` | `{ title, intro, sections[] }` целиком |
| `GET` | `/admin/users` | `UserDto[]` |
| `PATCH` | `/admin/users/{id}` | `{ name?, phone?, address?, role? }` |

Что важно не потерять при реализации:

- **Переименование категории обновляет `category_name` у товаров** — поле
  денормализовано, чтобы карточка не делала второй запрос.
- **Удаление непустой категории** без `move_to` — `422`, а не каскад.
  Товары не удаляются вместе с разделом.
- **Слаг товара уникален**; при смене слага заведите редирект, иначе ссылки
  протухнут.
- **`featured` у статьи — единственный**: публикация нового главного баннера
  снимает флаг с прежнего в той же транзакции.
...
«Повторить». Для нехватки прав — `403` с кодом `forbidden`.

### Расхождение, которое надо закрыть до реализации

`Order["status"]` на фронте — `new | processing | shipped | delivered |
cancelled`, а `OrderStatus` в схеме выше — `CREATED | PAID | SHIPPED |
DELIVERED | CANCELLED`. `dto.ts` статус **не маппит**, прокидывает как есть,
и типы это не поймают: `OrderDto.status` объявлен через `Order["status"]`.

Наборы к тому же про разное: `processing` — стадия обработки, `PAID` — факт
оплаты. Предложение: оставить один набор стадий и вынести оплату отдельным
полем (`paid_at`). Решать это надо сейчас, потому что с появлением заказов в
панели управления набор статусов становится рабочим процессом оператора.
...
**Шаг 5 — контент и панель управления.**
Перенести `src/data/content.ts` и `src/data/articles.ts` в БД, отдавать через
`/pages/{slug}` и `/articles`, реализовать `/admin/*`. Фронт готов целиком:
панель работает на моке, включая правку товаров, категорий, статей, текстов
страниц и пользователей.

**Шаг 6 — заказы в панели.**
`GET /admin/orders`, смена статуса, письмо покупателю. Единственный раздел
панели, которого сейчас нет — управлять нечем, пока заказы не создаются
сервером.

**Шаг 7 — загрузка изображений.**
Фото товара, категории и баннера сейчас задаются ссылкой. Presigned PUT в
S3/MinIO плюс ресайз закрывают и наполнение каталога, и подмену
плейсхолдеров Unsplash.
